...
This is the official Opscode Knife plugin for Ninefold Compute (Nova). This plugin gives knife the ability to create, bootstrap, and manage instances in Ninefold Compute clouds.
...
In order to communicate with an Ninefold Compute cloud's EC2 API you will have to tell Knife about your Ninefold Compute cloud API endpoint, Ninefold Access Key and Secret Access Key.  The easiest way to accomplish this is to create some entries in your <tt>knife.rb</tt> file:
    ...
    ### Note: If you are not proxying HTTPS to the Ninefold EC2 API port, the scheme should be HTTP, and the PORT is 8773.
    knife[:ninefold_api_endpoint]      = "https://cloud.mycompany.com/service/Cloud"

If your knife.rb file will be checked into a SCM system (ie readable by others) you may want to read the values from environment variables:

    knife[:ninefold_access_key_id]     = "#{ENV['EC2_ACCESS_KEY']}"
    knife[:ninefold_secret_access_key] = "#{ENV['EC2_SECRET_KEY']}"
    knife[:ninefold_api_endpoint]      = "#{ENV['EC2_URL']}"
...
You also have the option of passing your Ninefold API Key/Secret into the individual knife subcommands using the <tt>-A</tt> (or <tt>--ninefold-access-key-id</tt>) <tt>-K</tt> (or <tt>--ninefold-secret-access-key</tt>) command options
    ...
    knife ninefold server create 'role[webserver]' -I ami-7000f019 -f m1.small -A 'Your Ninefold Access Key ID' -K 'Your Ninefold Secret Access Key' --ninefold-api-endpoint 'https://cloud.mycompany.com/v1.0'
